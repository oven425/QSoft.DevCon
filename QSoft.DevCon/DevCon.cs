    ...
    static public partial class DevConExtension
    {
        ...
        public static ReadOnlyCollection<DEVPROPKEY> AllPropertys(this (IntPtr dev, SP_DEVINFO_DATA devdata) src)
        {
            ...
            for(int i = 0; i < count; i++)
            {
                ...
                var key = Marshal.PtrToStructure<DEVPROPKEY>(ptr);
                ll.Add(key);
            }
            return ll.AsReadOnly();
#endif
        }
        ...
        const uint SPDRP_LOCATION_PATHS = (0x00000023);  // Device Location Paths (R)
        const uint SPDRP_BASE_CONTAINERID = (0x00000024);  // Base ContainerID (R)

        const uint SPDRP_MAXIMUM_PROPERTY = (0x00000025);  // Upper bound on ordinals

        const uint DICS_FLAG_GLOBAL = 0x00000001;  // make change in all hardware profiles
        const uint DICS_FLAG_CONFIGSPECIFIC = 0x00000002;  // make change in specified profile only
        const uint DICS_FLAG_CONFIGGENERAL = 0x00000004;  // 1 or more hardware profile-specific
        const uint DIREG_DEV = 0x00000001;         // Open/Create/Delete device key
        const uint DIREG_DRV = 0x00000002;        // Open/Create/Delete driver key
        const uint DIREG_BOTH = 0x00000004;        // Delete both driver and Device key
